...
CREATE (saint_rémy_de_provence:CITY {name: 'Saint-Rémy-de-Provence', region: 'Provence-Alpes-Côte d\'Azur', country: 'France', latitude: 43.79, longitude: 4.8325})
...
CREATE (château_malromé:CITY {name: 'Château Malromé', country: 'France', latitude: 50.883333, longitude: 8.016667})
...
CREATE (arles_stage)-[:MOVED_TO]->(saint_rémy_de_provence_stage:STAGE {from: 188905, to: 189005 })
CREATE (saint_rémy_de_provence_stage)-[:IN]->(saint_rémy_de_provence)

CREATE (saint_rémy_de_provence_stage)-[:MOVED_TO]->(auvers_sur_oise_stage:STAGE {from: 189005, to: 189007 })
...
CREATE (toulouse_lautrec)-[:DIED_IN]->(château_malromé)
...
CREATE (bernard)-[:BELONGS_TO]->(post-impressionism)
...
CREATE (pissarro)-[:BELONGS_TO]->(post-impressionism)
